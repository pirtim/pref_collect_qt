...
from pref_collect_gui import Ui_MainWindow
...
class MyFirstGuiProgram(Ui_MainWindow):
    ...
    def updatePref(self, side):
        print('1.9: ', side) 
        print('1.99: ', next(self.line_gen)) 
        new = self.line_gen.send(side == 'right')
        print('3: ', new)
        if type(new) == int:
            print('4: ', self.result)
            try:
                ...
                self.toolButton_left.setEnabled(False)
                self.toolButton_draw.setEnabled(False)
                self.toolButton_right.setEnabled(False)
            ...
            new_new = next(self.line_gen)
            print("7: ", new_new)
            self.first_item = new_new[0]
            self.label_left.setText(self.first_item)
        else:
            self.first_item = new[0]
            self.label_left.setText(self.first_item)
            self.second_item = new[2]
            self.label_right.setText(self.second_item)
